###
# Uploader for custom app csv data.
...
class CustomAppTransformerUploader:
    ...
    def update_custom_app_config(self, provider_id: str, provider_name: str, file_id: str,
        ...
        current_config['inventoryFileId'] = file_id
        current_config['inventoryFileType'] = file_type
...
def parse_arguments() -> argparse.Namespace:
    ...
    Example:
        python3 sdk/samples/customapp_csv_uploader.py --app_name="foo" --file_name=samples/foo.csv
    """
...
def main() -> None:
    ...
    setup_logging()
    logger.info("Uploading file %s to apps %s", args.file_name, args.app_name)
    uploader = CustomAppTransformerUploader(args.as_api_endpoint, args.as_gql_endpoint)
    uploader.upload_and_update(args.app_name, args.file_name)
    logger.info("Successfully uploaded file %s and updated custom app %s successfully",
                args.file_name, args.app_name)
